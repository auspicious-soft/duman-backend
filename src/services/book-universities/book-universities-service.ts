						...
						try {
							...
							if (Array.isArray(authors)) {
								if (searchLanguage) {
									...
									authNames = authors.flatMap((author) => (author && author.name ? Object.values(author.name).map((val) => String(val || "").toLowerCase()) : []));
								}
							}

							// Check if any name includes the search query
							return prodNames.some((name) => typeof name === "string" && name.includes(searchQuery)) || authNames.some((name) => typeof name === "string" && name.includes(searchQuery));
						} catch (err) {
...
export const getBookUniversityCategoryService = async (user: any, payload: any, res: Response) => {
	...
	const uniqueCategories = categories.filter((value, index, self) => index === self.findIndex((t) => t && t._id && value && value._id && t._id.toString() === value._id.toString()));
	...
	if (payload.description) {
		...
		filteredCategories = uniqueCategories.filter((category) => {
			try {
				...
				// Check if any name includes the search query
				const result = categoryNames.some((name) => typeof name === "string" && name.includes(searchQuery));
	...
	if (payload.description) {
		...
		filteredAuthors = uniqueAuthors.filter((author) => {
			try {
				...
				// Check if any name includes the search query
				return authorNames.some((name) => typeof name === "string" && name.includes(searchQuery));
			} catch (error) {
...
export const getPopularCoursesBookUniversityService = async (payload: any, user: any, res: Response) => {
  ...
  const sortedResult = sortBooks(bookStudy, payload.sorting, userData?.productsLanguage, userData?.language);
...
export const getBookUniversityNewbookService = async (user: any, payload: any, res: Response) => {
	...
	const newBooks = await bookUniversitiesModel
		.find({
			createdAt: { $gte: sixMonthsAgo },
		})
		.populate({
