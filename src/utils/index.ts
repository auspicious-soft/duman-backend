...
export const productLanguages = async (book: any): Promise<string[]> => {
  const languages = new Set<string>();

  console.log('book?.file: ', book?.file);
  const addFromFileMap = () => {
    if (book?.file instanceof Map) {
      for (const [lang, value] of book.file.entries()) {
        if (value) languages.add(lang);
      }
    }
  };

  const addFromChapters = async (model: any) => {
    const chapters = await model
    .find({ productId: book._id })
    .select("lang")
    .lean();
    console.log('chapters: ', chapters);

    chapters.forEach((c: any) => {
      if (c?.lang) languages.add(c.lang);
    });
  };
  ...
  if (
    book?.type === "podcast" ||
    book?.type === "video-lecture" ||
    book?.format === "e-book"
  ) {
    addFromFileMap();
  }

  // Audiobook (audio chapters)
  if (book?.format === "audiobook" || book?.format === "both") {
    await addFromChapters(audiobookChaptersModel);
  }

  // Course lessons
  if (book?.type === "course") {
    await addFromChapters(courseLessonsModel);
  }
...
export const notificationMessages: any = {
	eng: {
		Author_Created: {
			...
			description: "A new author has been added.",
		},
		Product_Created: {
	...
	kaz: {
		Author_Created: {
			title: "Автор Создан",
			description: "Новый автор был добавлен.",
		},
		Product_Created: {
			title: "Продукт Создан",
			description: "Новый продукт был добавлен.",
		},
		Publisher_Created: {
	...
	rus: {
		...
		Product_Created: {
			title: "Продукт Создан",
			description: "Новый продукт был успешно создан.",
		},
		Publisher_Created: {
